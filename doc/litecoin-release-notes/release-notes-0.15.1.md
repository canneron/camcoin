Camcoin Core version *0.15.1* is now available from:

  <https://download.camcoin.org/camcoin-0.15.1/>
...
A number of changes to the way Camcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 camtoshis per kB (0.00001 XCAM). This is to prepare for dropping the minimum transaction fee to 0.1 camtoshis per kB (0.0001 XCAM) in the next release.
